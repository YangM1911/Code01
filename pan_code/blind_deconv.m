function [kernel, interim_latent] = blind_deconv(y, gammaL_pixel, gammaL_grad, opts)
...
for s = num_scales:-1:1
  ...
  if (s == num_scales)
    ...
    if threshold<gammaL_grad/10&&threshold~=0;
        gammaL_grad = threshold;
        %lambda_pixel = threshold_image_v1(ys);
        gammaL_pixel = gammaL_grad;
    end
  end
  %-----------------------------------------------------------%
  [ks, gammaL_pixel, gammaL_grad, interim_latent] = blind_deconv_main(ys, ks, gammaL_pixel,...
      gammaL_grad, threshold, opts);
